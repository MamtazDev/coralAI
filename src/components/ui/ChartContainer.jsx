...
const ChartContainer = () => {
  ...
  const coral = selectedImages? Number(selectedImages.percentages["hard corals"].toFixed(2))+ Number(selectedImages.percentages["soft corals"].toFixed(2)):Number(maskedImages[0].percentages["hard corals"].toFixed(2))+ Number(maskedImages[0].percentages["soft corals"].toFixed(2))

  // console.log(chart1Data, "chart");

  const chart2Data = [
    { name: "Coral", value: coral },
    { name: "Non-coral", value: 100-coral },
  ];
      ...
      <div>
        ...
        <div className="flex justify-center gap-10">
          <div className=" flex gap-2 items-center">
            ...
            <p className=" font-[700]">{chart2Data[0].value.toFixed(2)} %</p>
          </div>
          <div className="flex gap-2 items-center">
            <div className="h-[16px] w-[16px] bg-[#f9bf59] rounded-full"></div>
            <p className="text-[#cbcace] font-[600]">Non-coral</p>
            <p className=" font-[700]">{chart2Data[1].value.toFixed(2)}%</p>
          </div>
        </div>
      </div>
    </div>
